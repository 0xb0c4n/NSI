...
def voyageur(villes, dist, depart):
    ...
    courante = depart
    total_dist = 0
    tab_villes = [villes[depart]]
    for _ in range(n-1):
        visitees[courante] = True
        suivante = plus_proche(courante, dist, visitees)
        total_dist += dist[courante][suivante]
        tab_villes.append(villes[suivante])
        courante = suivante
    return [tab_villes, total_dist*100]
...
# distances en centaines de miles
plane_dist=[
...
print(voyageur(cities, plane_dist, 0))